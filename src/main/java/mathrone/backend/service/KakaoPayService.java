...
import mathrone.backend.domain.kakaoPay.*;
...
//
    ...
    public KakaoPaymentApproveResponse approveForKakaoPayment(KakaoPaymentApproveRequest kakaoPaymentApproveRequest){
        ...
        try {
            ...
            // 바디 만들기 (HashMap 사용 불가!)
            MultiValueMap<String, Object> params = new LinkedMultiValueMap<>();
            params.add("cid", kakaoPaymentApproveRequest.getCid());
            params.add("tid", kakaoPaymentApproveRequest.getTid());
            params.add("partner_order_id", kakaoPaymentApproveRequest.getPartner_order_id());
            params.add("partner_user_id", kakaoPaymentApproveRequest.getPartner_user_id());
            params.add("pg_token", kakaoPaymentApproveRequest.getPg_token());
            ...
            // Http 요청하고 리턴값을 response 변수로 받기
            ResponseEntity<String> apiResponseJson = rt.exchange(
                    "https://kapi.kakao.com/v1/payment/approve", // Host
                    HttpMethod.POST, // Request Method
                    kakaoTokenRequest,    // RequestBody
                    String.class
            );    // return Object
...
//            System.out.println(apiResponseJson.getBody().toString());
            ...
            KakaoPaymentApproveResponse kakaoPaymentApproveResponse = objectMapper.readValue(
                    apiResponseJson.getBody(), new TypeReference<KakaoPaymentApproveResponse>() {
                    });
